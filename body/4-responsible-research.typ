...
#todo[Move to conclusion?] Before discussing the results, we want to spend a few paragraphs on a somewhat unorthodox reflection (which a busy reader can skip) about making responsible research. For, despite most of the "usual suspects" of unethical investigation are not present in this paper (there's no Machine Learning, no training dataset, no massive use of computing/natural resources, no direct consequences in highly critical infrastructure, etc) there are considerations we can take about the effect of the results presented in the paper.
...
The presented research is pretty self contained, so it is hard to find problem within. However, we can (and should) think about the bigger picture. For instance, the first line of the paper is an anecdote about how Amazon does 1 billion SMT queries a day @rungtaBillionSMTQueries2022. If we help to improve Z3, we might be indirectly helping Amazon, which is arguably considered widely unethical. So is the presented research unethical too by association?
...
// It is basically impossible to be completely ethical, by its most stringent definition, while living in current society. But, looking at the bigger picture, it is hard to argue that the research presented today does any more harm to society than any reasonable expectation of a person's participation in it.
